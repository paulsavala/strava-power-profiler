...
class ApiV3(object, metaclass=abc.ABCMeta):
    """
    ...
    def authorization_url(self, client_id, redirect_uri, approval_prompt='auto', scope=None, state=None):
        ...
        return urllib.parse.urlunsplit(('https', self.server, '/oauth/authorize', urlencode(params), ''))
    ...
    def _resolve_url(self, url):
        if not url.startswith('http'):
            url = urllib.parse.urljoin('https://{0}'.format(self.server), self.api_base + '/' + url.strip('/'))
        return url
    ...
    def get(self, url, check_for_errors=True, **kwargs):
        ...
        params = dict([(k, v) for k, v in list(kwargs.items()) if not k in referenced])
